...
const VideoPlayer = () => {
  ...
  const handleNextStep = async () => {
    const response = await updateVideoWatched().unwrap();
    if (response?.status === 200) {
      dispatch(getUserProfile());
    }
  };

  const isProfileData = profile && profile && Object.keys(profile).length;
  useEffect(() => {
    if (
      profile?.approve_status === "pending" &&
      profile?.video_status === "watched"
    ) {
      navigate("/dashboard");
    }
  }, [isProfileData]);
  ...
  return (
    ...
    >
      <div className="flex justify-between px-7 pt-6 pb-5 mx-2 border-b-2 font-medium text-[25px] ">
        Orientation video
      </div>
