...
import { handlehtmlsend, updateReportImage, uploadVideoFiles } from '../../services/reportsInfo';
...
const ArticleCustom = ({ onSave, onCancel }) => {
    ...
    useEffect(() => {
        ...
        if (videoUrl) {
            setBlocks(blocks.map(block => {
                if (block.type === BlockTypes.VIDEO && block.content instanceof File) {
                    console.log("Setting video URL in block:", videoUrl);
                    return {
                        ...block,
                        content: videoUrl
                    };
    ...
    useEffect(() => {
        ...
        if (url && activeUploadBlockId) {
            setBlocks(blocks.map(block => {
                if (block.id === activeUploadBlockId && block.content && block.content.imageFile) {
                    return {
                        ...block,
                        content: {
                            ...block.content,
    ...
    const uploadImage = async (file) => {
        ...
        formData.append('isActive',false);
            ...
            if (response.payload?.message === "Created successfully.") {
                ...
                setUrl(imageUrl);
                console.log("URL set in state:", imageUrl);
                return imageUrl;
            }
        } catch (error) {
    ...
    const uploadVideo = async (file) => {
        try {
            ...
            // if (response.payload?.message === "Created successfully.") {
                ...
                console.log("Video URL received from server:", videoUrl);
        ...
        try {
            ...
            setBlocks(newBlocks);
                ...
                if (videoUrl) {
                    ...
                    setBlocks(newBlocks);
                    console.log("Block updated with video URL:", videoUrl);
                }
            }
        ...
        if (type === 'image') {
            try {
                if (content instanceof File) {
                    console.log("Starting upload for block:", blockId);
    ...
    const handleSave = () => {
        ...
        const htmlContent = blocks.map((block) => {
            ...
            if (block.type === BlockTypes.TEXT) {
                ...
                const imageHtml = block.content.image ? `<img src="${block.content.image}" style="width: 100%; height: 200px; object-fit: cover; border-radius: 4px;" />` : '';
                const flexDirection = block.imagePosition === 'right' ? 'row' : 'row-reverse';
        ...
        dispatch(handlehtmlsend({
            id: 1,
            data: formValue
        }))
            .then((response) => {
                if (response?.meta?.requestStatus === "fulfilled") {
                    console.log("HTML content saved successfully:", response);
...
setBlocks([
    ...
    const renderBlock = (block, index) => {
        ...
        const previewUrl = block.content instanceof File ? URL.createObjectURL(block.content) : block.content;
                    ...
                    {blocks.map((block, index) => (
                        <div key={block.id} className="block-preview">
                            ...
                            {block.type === 'text_image' && (
                                <div className={`flex gap-8 items-start ${block.imagePosition === 'right' ? 'flex-row' : 'flex-row-reverse'}`}>
                                    <div 
