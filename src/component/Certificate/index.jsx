...
export default function Certificate() {
  ...
  const [actionTab, setActiveTab] = useState(
    selectedRequestedTab && role === "mentor"
    ...
    if (role) {
      dispatch(
        getCertificateList(
          role === "admin"
            ? `?status=${
                role === "admin" ? "approved" : requestTab
            ...
            : role === "mentor"
            ? `?status=${actionTab}&page=${paginationModel?.page + 1}&limit=${
            ...
            <div className="cursor-pointer flex items-center justify-center h-full relative">
              <span
                ...
                style={{
                  background:
                    requestStatusColor[
                      role === "mentee" && !params.row?.certificate_rating
                        ? "review"
                        : params.row.status
                    ]?.bg || "",
                  lineHeight: "30px",
              ...
              >
                {" "}
                {!params.row?.certificate_rating && role === "mentee"
                  ? "Please provide your rating"
                  : certificateRequestStatusText[params.row.status]}
              </span>
            </div>
          </>
        );
            ...
            >
              ...
              {role === "admin" && (
                <MenuItem
                  onClick={() =>
                    navigate(
                      `/certificate_mentees/${seletedItem.id}?type=approved&breadcrumbsType=${requestTab}`,
                      {
                        state: {
                          rowId: seletedItem?.id,
                          status: seletedItem?.status,
  ...
  if (role === "admin") {
    certificateColumn = certificateColumn.filter(
      (column) => !mentorFields.includes(column.field)
    );
  }
