...
export default function MenteeTaskList() {
    ...
    const handleSearch = (e) => {
        ...
        let query = { mentee_id: searchParams.get('mentee_id'), Program_id: searchParams.get('program_id') }
    ...
    useEffect(() => {
        if (searchParams.has('mentee_id') && searchParams.has('program_id')) {
            dispatch(getMenteeAllTask({ mentee_id: searchParams.get('mentee_id'), Program_id: searchParams.get('program_id') }))
        }
                                            ...
                                            <div className='flex justify-between items-center w-full'>
                                                <p className='notification-message'>
                                                    <p className='font-normal flex gap-2 items-center'>
                                                        <p>{list.task_name}</p>
                                                        <p className={`px-7 py-1 task_action_${list?.result?.toLowerCase()}`}>{list.result}</p>
                                                    </p>
                                                    <p className='text-[13px] pt-4 leading-6'>
                                                        {list.task_description}
                                                    </p>
                                                </p>
                                                <button className='py-3 px-2 w-[200px] text-white' style={{ background: 'rgba(29, 91, 191, 1)', borderRadius: '6px' }}
                                                    onClick={() => navigate(`/mentor-tasks-details/${list.id}?mentee_id=${list.mentee_id}`)}>
                                                    View Task</button>
                                            </div>
                                        </li>
                                    )
