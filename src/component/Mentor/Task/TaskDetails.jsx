...
const MentorTaskDetails = () => {
  ...
  const onSubmit = (data) => {
    const apiData = {
      ...
      mentee_id: taskDetails.mentee_id,
          ...
          <>
            <div className="cursor-pointer flex items-center h-full relative">
              <span
                ...
                style={{
                  background: taskStatusColorNew[params?.row?.status]?.bg,
                  lineHeight: "30px",
                  borderRadius: "3px",
                  width: "110px",
                  height: "34px",
                  color: taskStatusColorNew?.[params?.row?.status]?.color,
                }}
              >
                ...
                <>
                  ...
                  {selectedItem?.status === "pending" && (
                    <>
                      <MenuItem
                        onClick={() =>
                          handleOpenConfirmPopup("pass", "", "pass")
                        }
                        className="!text-[12px]"
                      >
                        <img
  ...
  const handleEditTask = () => {
    ...
    const data = Object.fromEntries(
      Object.entries(menteeTaskList).filter(
    ...
    const constructedData = {
      ...
      assign_task_id: data?.id,
      list_mentees: data?.list_mentees,
      program_id: data?.program_id,
      program_duration: data?.duration,
      category_id: data?.category_id,
      mentor_id: data?.mentor_id,
      mentor_name: data?.mentor_name,
      state_date: data?.start_date,
    };
  ...
  const handleCancelAllMentee = (reason = "") => {
    ...
    const payload = {
      task_id:
        selectedTab === "new" && newType === "newTab"
          ? selectedItem?.id
          : selectedTab === ""
          ? selectedItem?.id
          : menteeTaskList?.assign_task_id,
      type:
        selectedTab === "new" && newType === "newTab"
          ? "cancel_one_task"
          : selectedTab === ""
  ...
  return (
    <div className="px-9 py-9">
        ...
        <div className="px-4">
                  ...
                  return (
                    <Grid item xs={12}>
                      <Grid container>
                        ...
                        >
                          <Typography className="px-6 py-4 font-medium whitespace-nowrap !text-[#18283D] !text-[14px]">
                            {e?.title}
                          </Typography>
                        </Grid>
                        <Grid
                          ...
                          sx={{
                            borderBottom:
                              len?.length - 1 === i && "1px solid #00AEBD",
                        ...
                        >
                          <Typography
                            className="px-6 py-4 text-white !text-[14px] !bg-[#00AEBD]"
                            sx={{
                              borderBottom:
                                len?.length - 1 === i && "1px solid #00AEBD",
                              borderTopRightRadius: i === 0 && "3px",
                    ...
                    <Grid item xs={12}>
                      <Grid container>
                        ...
                        <Grid
                          ...
                          sx={{
                            borderBottom:
                              len?.length - 1 === i && "1px solid #1D5BBF",
                            borderTopRightRadius: i === 0 && "3px",
                            borderBottomRightRadius:
                              len?.length - 1 === i && "3px",
                          }}
                        >
                          <Typography
                            className="px-6 py-4 text-white !text-[14px] !bg-[#1D5BBF]"
                            sx={{
                              borderBottom:
                                len?.length - 1 === i && "1px solid #1D5BBF",
                              borderTopRightRadius: i === 0 && "3px",
          ...
          <Box className="!border !border-[#1D5BBF80] rounded-[3px] mt-5">
            <Stack
              ...
              className="!border-b-2 !border-[#1D5BBF80] px-[35px] py-[22px]"
            >
              <Typography
          ...
          <Box mt={"48px"}>
                      ...
                      label={
                        <Typography
                          className={`!text-[14px] text-[${
                            selectedTab === e.key ? "#1D5BBF" : "#18283D"
                          }] 
                                                    capitalize -pb-[8px]`}
                          sx={{ fontWeight: 400 }}
                        >
            ...
            {selectedTab === "pending" &&
              menteeTaskList?.results?.length > 0 && (
                <Stack
                  direction={"row"}
                  alignItems={"center"}
      ...
      <CancelPopup
        open={confirmPopup?.cancel}
        header={confirmPopup?.title}
        ...
        <div className="popup-content w-2/6 md:w-2/4 sm:w-2/4 bg-white flex flex-col gap-2 h-[330px] justify-center items-center">
          ...
          <div className="flex justify-center" mt={4}>
            <div className="flex gap-6 justify-center align-middle">
              <Button
                btnCls="w-[200px] !border !border-[#18283D80] !text-[#18283D]"
                btnName={"Cancel"}
                btnCategory="secondary"
                onClick={() => handleCloseConfirmPopup("pass")}
              />
              <Button
                btnType="button"
                btnCls="w-[200px]"
                btnName={"Pass"}
                style={{ background: "#16B681", color: "#fff" }}
                btnCategory="secondary"
                onClick={() => {
