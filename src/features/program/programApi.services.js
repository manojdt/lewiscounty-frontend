...
export const programsApi = rtkQueryApiServices.injectEndpoints({
    endpoints: (builder) => ({
        ...
        getProgramDetailsById: builder.query({
            query: ({ id, role }) => ({
                url: role === "admin" ? `program/admin-program/${id}` : `programs/${id}`,
            }),
        }),

        getSpecificProgramDetails: builder.query({
            query: ({ id, requestId = '' }) => `programs/${id}?request_id=${requestId}`,
            transformResponse: (response) => response.program,
        }),        
        ...
        getMenteeProgramCount: builder.query({
            query: (query) => {
                ...
                return `mentee_program/count_program?${new URLSearchParams(filteredQuery).toString()}`;
            },
        }),
    }),
});
